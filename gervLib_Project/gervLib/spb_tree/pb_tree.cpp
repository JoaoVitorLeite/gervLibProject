...
template <class type>
unsigned * PB_Tree<type>:: Zconvert(int * dist)
{
...
template <class type>
void PB_Tree<type>::Zconvert(BasicArrayObject<type> *o)
{
	int keysize = (num_piv * bits)/32;
	if((num_piv * bits)%32!=0)
		keysize = keysize +1;
	...
	{
		if(eps >0)
            dists[i] = (int) floor(df->getDistance(*o, ptable[i])/eps);
		else
            dists[i]= (int)df->getDistance(*o, ptable[i]);
...
template <class type>
int* PB_Tree<type>::R_Zconvert(unsigned * okey)
{
	int keysize = (num_piv * bits)/32;
	if((num_piv * bits)%32!=0)
		keysize = keysize +1;
...
template <class type>
void PB_Tree<type>::Hconvert(BasicArrayObject<type> *o)
{
	int keysize = (num_piv * bits)/32;
	if((num_piv * bits)%32!=0)
		keysize = keysize +1;
...
template <class type>
unsigned * PB_Tree<type>:: Hconvert ( unsigned* hcode,  unsigned* point, int DIMS ) {
...
template <class type>
unsigned * PB_Tree<type>:: R_Hconvert ( unsigned* point, unsigned* hc, int DIMS )
{
...
template <class type>
void PB_Tree<type>::readptable(char *pname, Dataset<type>* data)
{
//	ifstream in1(pname,ios::in);
//    ptable = new BasicArrayObject<type>[num_piv];
//    size_t d1, d2;
//    double d3;
//	for(int i =0;i<num_piv;i++)
//	{
//        in1>>d1>>d2;
//        ptable[i] = BasicArrayObject<type>(d1, d2);
...
//	in1.close();
    ...
    if(pvtMethod != nullptr && df != nullptr && data != nullptr)
    {

        pvtMethod->generatePivots(data, df, num_piv);

    }
    else
    {
        ptable = new BasicArrayObject<type>[num_piv];
        Dataset<type>* dataPivots = new Dataset<type>();

        if constexpr (std::is_same<type, std::vector<char>>::value)
        {

            Dataset<type>::loadTextDataset(dataPivots, pname, " ");

        }
        else
        {

            Dataset<type>::loadNumericDataset(dataPivots, pname, " ");
        ...
        for(int i = 0; i < num_piv; i++)
        {
            ...
            ptable[i] = dataPivots->getFeatureVector(i);
...
template <class type>
void PB_Tree<type>::H_bulkload(BasicArrayObject<type> * O, int o_num)
{
	vector<HEntry> H;
	for(int i =0;i<o_num;i++)
	{
...
template <class type>
void PB_Tree<type>::bulkload(BasicArrayObject<type> * O, int o_num)
{	
	vector<TEntry> v;
	for(int i =0;i<o_num;i++)
	{
	...
	if(f!=NULL)
	{
		for(int i=0;i<o_num;i++)
		{
            fprintf(f, "%d ", (int)O[v[i].id].getOID());
...
template <class type>
int PB_Tree<type>::idist(int * q, int* mi, int* ma)
{
	int dist =0;
	int temp;
	for(int i =0;i<num_piv;i++)
...
template <class type>
double PB_Tree<type>::BFkNN(BasicArrayObject<type> *q, int k)
{
    df->resetStatistics();
	int * dists = new int[num_piv]; 
	for(int i =0;i<num_piv;i++)
	{
		if(eps >0)
            dists[i] = (int) floor(df->getDistance(*q, ptable[i])/eps);
		else
            dists[i]= (int) df->getDistance(*q, ptable[i]);
	}
	int addition =0;
	int temp;
	if(eps>0)
	...
	while(!queue.empty())
	{
		...
		{
			SortEntry se;
            BasicArrayObject<type>* o = getobject(e.ptr);
            se.key=df->getDistance(*q, *o);
			if(se.key<=kdist)
			{
                se.id=o->getOID();
				kNN.push_back(se);
    ...
    cout << "DIST CNT = " << df->getDistanceCount() << "\n\n";
    for(size_t i = 0; i < kNN.size(); i++)
    {

        cout << kNN[i].id << " / " << kNN[i].key << endl;
...
template <class type>
bool PB_Tree<type>::inregion(int* point, int* oqmin, int* oqmax)
{
	for(int i =0;i<num_piv;i++)
	{
...
template <class type>
int PB_Tree<type>::edist(int * query, int* o)
{
	int ed = 0;
	int temp;
	for(int i =0;i<num_piv;i++)
...
template <class type>
int PB_Tree<type>::edist(int * query, unsigned * o)
{
	int ed = 0;
	int temp;
	for(int i =0;i<num_piv;i++)
...
// return 0, 1, 2 or -1
template <class type>
int PB_Tree<type>::determine(int * min1, int * max1, int* min2, int* max2)
{
	int c1, c2, c3;
	c1= c2=c3=0;
	for(int i =0;i<num_piv;i++)
...
template <class type>
void PB_Tree<type>::RRQH_new(B_Node* node, vector<int>* result, int* qmin, int* qmax, int* min, int *max, bool flag, bool validate, int* query, int radius)
{
	if (node->level != 0)
	{
		if (flag)
...
template <class type>
int PB_Tree<type>::ImprovedRQ_new(BasicArrayObject<type> * q, double radius)
{
	int * dists = new int[num_piv]; 
	...
	for(int i =0;i<num_piv;i++)
	{
        tpdists[i] = df->getDistance(*q, ptable[i]);
    ...
    BasicArrayObject<type> * o= NULL;
	for(int j =0;j<result->size();j++)
	{
		o = getobject((*result)[j]);
        if(df->getDistance(*q, *o)-radius<0.000001)
		{
            RQ.push_back(o->getOID());
		}
		delete o;
	}
	result->clear();
...
template <class type>
BasicArrayObject<type>* PB_Tree<type>::getobject(int ptr)
{
    ...
    BasicArrayObject<type>* o = new BasicArrayObject<type>();
    //o->unserialize((unsigned char*)buffer[0]);
...
template class PB_Tree<double>;
template class PB_Tree<std::vector<char>>;
