...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        DistanceFunction<BasicArrayObject<std::vector<char>>>* df = nullptr;
        Pivot<std::vector<char>>* pvt = nullptr;
        ...
        if((dataset_train_cardinality != nullptr) && (dataset_train_dimensionality != nullptr))
        {
            ...
            Dataset<std::vector<char>>::loadTextDataset(train, *dataset_train, *dataset_train_cardinality, *dataset_train_dimensionality);
        ...
        else
        {
            ...
            if(dataset_train_separator != nullptr)
            {
                ...
                Dataset<std::vector<char>>::loadTextDataset(train, *dataset_train, *dataset_train_separator);

            }
            else
            {

                Dataset<std::vector<char>>::loadTextDataset(train, *dataset_train, ",");
        ...
        if((dataset_test_cardinality != nullptr) && (dataset_test_dimensionality != nullptr))
        {
            ...
            Dataset<std::vector<char>>::loadTextDataset(test, *dataset_test, *dataset_test_cardinality, *dataset_test_dimensionality);
        ...
        else
        {
            ...
            if(dataset_test_separator != nullptr)
            {
                ...
                Dataset<std::vector<char>>::loadTextDataset(test, *dataset_test, *dataset_test_separator);

            }
            else
            {

                Dataset<std::vector<char>>::loadTextDataset(test, *dataset_test, ",");
        ...
        if(distanceFunction == nullptr)
        {
            ...
            df = new EditDistance<BasicArrayObject<std::vector<char>>>();

        }
        else
        {

            if(*distanceFunction == "EDIT")
                df = new EditDistance<BasicArrayObject<std::vector<char>>>();
            else
            {
        ...
        if(pivot_type != nullptr)
        {
            ...
            {
                pvt = new RandomPivots<std::vector<char>>();
            }
            else if(*pivot_type == "GNAT")
            {
                pvt = new GnatPivots<std::vector<char>>();
            }
            else if(*pivot_type == "CONVEX")
            {
                pvt = new ConvexPivots<std::vector<char>>();
            }
            else if(*pivot_type == "KMEDOIDS")
            {
                pvt = new KmedoidsPivots<std::vector<char>>();
            }
            else if(*pivot_type == "MAXSEPARETED")
            {
                pvt = new MaxSeparetedPivots<std::vector<char>>();
            }
            else if(*pivot_type == "MAXVARIANCE")
            {
                pvt = new MaxVariancePivots<std::vector<char>>();
            }
            else if(*pivot_type == "SELECTION")
            {
                pvt = new SelectionPivots<std::vector<char>>();
            }
            else if(*pivot_type == "PCA")
            {
                pvt = new PCAPivots<std::vector<char>>();
            }
            else if(*pivot_type == "SSS")
            {
                pvt = new SSSPivots<std::vector<char>>();
            }
            else if(*pivot_type == "FFT")
            {
                pvt = new FFTPivots<std::vector<char>>();
            }
            else if(*pivot_type == "HFI")
            {
                pvt = new HFIPivots<std::vector<char>>();
            }
            else if(*pivot_type == "IS")
            {
                pvt = new ISPivots<std::vector<char>>();
            }
            else if(*pivot_type == "WDR")
            {
                pvt = new WDRPivots<std::vector<char>>();
            }
            else
            {
        ...
        if(*index == "OMNIKDTREE")
        {
            ...
            OmniKdTree<std::vector<char>>* index_query = new OmniKdTree<std::vector<char>>(train, df, pvt, *num_per_leaf);
        ...
        else if(*index == "VPTREE")
        {
            ...
            VpTree<std::vector<char>, DistanceFunction<BasicArrayObject<std::vector<char>>>>* index_query = new VpTree<std::vector<char>, DistanceFunction<BasicArrayObject<std::vector<char>>>>(false, 0.0, *num_per_leaf, pvt, train, df);
            ...
            for(size_t k = 5; k <= *k_max; k += 5)
            {
                ...
                for(size_t x = 0; x < max_query; x++)
                {
                    ...
                    ans = new Dataset<std::vector<char>>();
                    df->resetStatistics();
                    auto start = std::chrono::steady_clock::now();
        ...
        else if(*index == "PMTREE")
        {
            ...
            PM_Tree<std::vector<char>>* index_query = new PM_Tree<std::vector<char>>(train, df, pvt, *num_per_leaf, *num_pivots);
